...
function App() {
    ...
    const btn = () => {
        ...
        // document.getElementById("d01").innerText = name;    // 자바스크립트, ajax 데이터값을 넣는 방법
    }
    ...
    return (
        <div className='main'>
            <h2>게시판</h2>
            <div id="d01">{name}</div>
            <div id="d01">{ename}</div>
            <br />
            <input type='text' name='id' className='id' onChange={(event)=>setId(event.target.value)} value={id} placeholder='아이디를 입력하세요.' />
            <br />
            <input type='text' name='pw' className='pw' onChange={(event)=> setPw(event.target.value)} value={pw} onKeyUp={pwBtn} placeholder='패스워드를 입력하세요.' />
            <br />
